...
public class BookmarkController {
  ...
  public ResponseEntity<SliceResponse<BookmarkResponse>> getMyBookmarks(
      ...
      @Parameter(description = "페이지 크기", example = "20") @RequestParam(defaultValue = "20")
          int size) {
    return ResponseEntity.ok(bookmarkService.getMyBookmarks(principal.userId(), page, size));
  }
  ...
  public ResponseEntity<Void> addBookmark(
      ...
      @Parameter(description = "시설 ID", required = true, example = "FAC001") @PathVariable
          String facilityId) {
    bookmarkService.addBookmark(principal.userId(), facilityId);
  ...
  public ResponseEntity<Void> removeBookmark(
      ...
      @Parameter(description = "시설 ID", required = true, example = "FAC001") @PathVariable
          String facilityId) {
    bookmarkService.removeBookmark(principal.userId(), facilityId);
    return ResponseEntity.ok().build();
  }
  ...
  public ResponseEntity<BookmarkStatusResponse> getBookmarkStatus(
      ...
      @Parameter(description = "시설 ID", required = true, example = "FAC001") @PathVariable
          String facilityId) {
    return ResponseEntity.ok(bookmarkService.getBookmarkStatus(principal.userId(), facilityId));
  }
}
